...
Euler's formula extends naturally with complex indices:
\[
...
directions. IVNA's indexed infinities serve an analogous function:
where a blow-up parametrizes approach directions geometrically via an
exceptional divisor, IVNA parametrizes them arithmetically via the
index. The key difference is that blow-ups are geometric constructions
requiring scheme-theoretic machinery, while IVNA provides a closed-form
arithmetic on the resolved objects, making it applicable in discrete
and computational settings where geometric resolution is unavailable.
